...
int TcpProc(DNode **ppstDNode, int iTimeOut) {
    ...
    switch (iRet) {
        case PACK_LEN_ERR:
            tLog(ERROR, "收到描述字[%d][%s]报文长度错误,关闭连接.", iFd, pstMultiItem->sClientInfo);
    ...
    sprintf(sMsg, "[%s]接收请求报文", GetSvrId());
    ...
    tLog(INFO, "增加键值[%s]至交易链表项成功.", stTransItem.sUniqueKey);
    ...
    switch (iRet) {
        case MSG_ERR:
            ...
            break;
        case MSG_SUCC:
            tLog(INFO, "键值[%s]发送消息成功.", stTransItem.sUniqueKey);
            break;
    }
    return 0;
}
...
int PipeProc(DNode *pstDNode, int iTimeOut) {
    ...
    iLen = atoi(sLen);
    tLog(DEBUG, "读取管道内容长度:%d", iLen);
    iLen = read(iFd, sRedisTran, iLen);
    //if (iLen <= 0 || iLen != sizeof (sRedisTran)) {
    if (iLen <= 0) {
    ...
    tLog(DEBUG, "key[%s]", sUniqueKey);
    pstTransNode = GetTransLink(sUniqueKey, &g_stTransLink, &g_stTransFree, GetTranTimeout()); //g_stModuleInfo.iTranTimeout );
    ...
    tAsc2Bcd(pstSndMultiItem->caData + 2, sMsg, strlen(sMsg), LEFT_ALIGN);
    //组报文头
    iLen = strlen(sMsg) / 2;
    pstSndMultiItem->caData[0] = iLen / 256;
    pstSndMultiItem->caData[1] = iLen % 256;
    pstTransItem->iLen = iLen + 2;
