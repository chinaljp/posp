...
int InitParam(int iArgc, char* pcArgv[]) {
    ...
    while ((iOpt = getopt(iArgc, pcArgv, "hH:l:L:b:B:s:S:n:N:c:C:")) != -1)
        switch (iOpt) {
            ...
            case 'c':
            case 'C':
                tStrCpy(g_sNoticeCnt, optarg, 10);
                break;
            case '?':
    ...
    if (g_sAppNum[0] == '\0'
            || g_sLogName[0] == '\0' || g_sSvrName[0] == '\0' || g_sNoticeCnt[0] == '\0' ) {
        fprintf(stderr, "Basic Usage: %s  -l [Log Name] -s [Svr Name]  -n [App Num] -c [Notice cnt]\n", pcArgv[0]);
        return -1;
    }
    fprintf(stderr, "%s  -s %s -n %s -l %s -b %s -c %s成功\n", pcArgv[0], g_sSvrName, g_sAppNum, g_sLogName, g_sLibName,g_sNoticeCnt);
    return 0;
}