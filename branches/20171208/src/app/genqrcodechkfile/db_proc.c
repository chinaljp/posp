...
void GetMsgType(char *pcOMsgType, char *pcTransCode) {
    if (strstr("020000,020002,024102,T20000,024100,M20000,M20002,021000", pcTransCode) != NULL) {
        strcpy(pcOMsgType, "0200");
    } else if (strstr("024000,024002", pcTransCode) != NULL) {
        strcpy(pcOMsgType, "0100");
    } else if (strstr("02B100,02B200,02W100,02W200,0AB100,0AB200,0AW100,0AW200,02Y100,02Y200", pcTransCode) != NULL) {
        strcpy(pcOMsgType, "1200");
    } else if (strstr("02B600,02W600,02W101,02W201,0AB101,0AB201,0AW101,0AW201,02Y600", pcTransCode) != NULL) {
        strcpy(pcOMsgType, "2200");
    }
}

int FindQrCurSettleDate(char *pcDate) {
    char sDate[8 + 1] = {0};
    int i = 0;
...
int writefilehead(FILE *fp, char *pcTransDate) {
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "select  sum(num),sum(amt1)-sum(amt2),sum(fee1)-sum(fee2) from "
            ...
            "where (a.settle_sys_trace,a.settle_trans_time) in(select sys_trace,transmit_time from b_cups_trans_detail where settle_date='%s') and  a.check_flag in ('Y','B') "
            //"where a.merch_order_no in(select merch_order_no from b_hxb_trans_detail where settle_date='%s') and  a.check_flag in ('Y','B') "
            /*
            "group by t.acct_flag  union select t.acct_flag,count(*) as num ,decode(t.acct_flag,'+',sum(a.amount)) as amt1 " 
            ",decode(t.acct_flag,'+',sum(a.amount)-sum(a.fee), 0) as fee1 "
            ",decode(t.acct_flag,'-',sum(a.amount), 0) as amt2 "
            ",decode(t.acct_flag,'-',sum(a.amount)-sum(a.fee), 0) as fee2 "
            "from b_inline_tarns_detail_his a "
            "join S_TRANS_CODE t on t.TRANS_CODE=a.TRANS_CODE and t.settle_flag='1' "
            "join b_merch_account b on a.merch_id=b.merch_id "
            "where a.merch_order_no in(select merch_order_no from b_hxb_trans_detail where settle_date='%s') and a.trans_date='%s' and  a.check_flag in ('Y','B') and a.trans_time >'235900' "
             */
            ...
            "join S_TRANS_CODE t on t.TRANS_CODE=a.TRANS_CODE and t.settle_flag='1' "
            "join b_merch_account b on a.merch_id=b.merch_id "
            "where a.merch_order_no in(select merch_order_no from b_wb_trans_detail where trans_date='%s') and  a.check_flag in ('Y','B') group by t.acct_flag "
             "union "
            "select t.acct_flag,count(*) as num ,decode(t.acct_flag,'+',sum(a.amount)) as amt1 "
            ",decode(t.acct_flag,'+',sum(a.amount)-sum(a.fee), 0) as fee1 "
            ",decode(t.acct_flag,'-',sum(a.amount), 0) as amt2 "
            ...
            "join S_TRANS_CODE t on t.TRANS_CODE=a.TRANS_CODE and t.settle_flag='1' "
            "join b_merch_account b on a.merch_id=b.merch_id "
            "where a.merch_order_no in(select merch_order_no from b_qrbank_trans_detail where trans_date='%s') and  a.check_flag in ('Y','B') group by t.acct_flag "
             "union "
            "select t.acct_flag,count(*) as num ,decode(t.acct_flag,'+',sum(a.amount)) as amt1 "
            ",decode(t.acct_flag,'+',sum(a.amount)-sum(a.fee), 0) as fee1 "
            ",decode(t.acct_flag,'-',sum(a.amount), 0) as amt2 "
...
int writefilebody(FILE *fp, char *pcTransDate, char *pcSettleDate) {
    ...
    FindQrCurSettleDate(sSettleDate);
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "select a.rrn,a.amount,b.account_uuid,a.trans_code "
            ...
            "join b_merch_account b on a.merch_id=b.merch_id "
            //"where  a.merch_order_no in(select merch_order_no from b_hxb_trans_detail where settle_date='%s') and a.check_flag in ('Y','B')", sTransDate);
            "where (a.settle_sys_trace,a.settle_trans_time) in(select sys_trace,transmit_time from b_cups_trans_detail where settle_date='%s') and a.check_flag in ('Y','B') "
            "union "
            "select a.rrn,a.amount,b.account_uuid,a.trans_code "
            ",a.sys_trace,a.merch_id,a.channel_merch_id,a.trans_date,a.trans_time, a.amount-a.fee "
            "from b_inline_tarns_detail_his a join S_TRANS_CODE t on t.TRANS_CODE=a.TRANS_CODE and t.settle_flag='1' "
            "join b_merch_account b on a.merch_id=b.merch_id "
            "where a.merch_order_no in(select merch_order_no from b_wb_trans_detail where trans_date='%s') and a.check_flag in ('Y','B')"            
            "union "
            "select a.rrn,a.amount,b.account_uuid,a.trans_code "
            ",a.sys_trace,a.merch_id,a.channel_merch_id,a.trans_date,a.trans_time, a.amount-a.fee "
            "from b_inline_tarns_detail_his a join S_TRANS_CODE t on t.TRANS_CODE=a.TRANS_CODE and t.settle_flag='1' "
            "join b_merch_account b on a.merch_id=b.merch_id "
            "where a.merch_order_no in(select merch_order_no from b_qrbank_trans_detail where trans_date='%s') and a.check_flag in ('Y','B')"           
            "union "
            "select a.rrn,a.amount,b.account_uuid,a.trans_code "
            ",a.sys_trace,a.merch_id,a.channel_merch_id,a.trans_date,a.trans_time, a.amount-a.fee "
            "from b_inline_tarns_detail_his a join S_TRANS_CODE t on t.TRANS_CODE=a.TRANS_CODE and t.settle_flag='1' "
            ...
            "from b_inline_tarns_detail_his a join S_TRANS_CODE t on t.TRANS_CODE=a.TRANS_CODE and t.settle_flag='1' "
            "join b_merch_account b on a.merch_id=b.merch_id "
            "where  a.merch_order_no in(select merch_order_no from b_hxb_trans_detail where settle_date='%s') and a.trans_date='%s' and a.check_flag in ('Y','B')   and trans_time >'235900' " , sTransDate, sBTransDate, sBTransDate);
             */
    tLog(INFO, "[%s]", sSqlStr);
    if (tExecute(&pstRes, sSqlStr) < 0) {
        tLog(ERROR, "sql[%s] err!", sSqlStr);
