...
int tCall(char *pcQSvrId, MsgData *pstMsgData) {
    ...
    tLog(WARN, "Send Msg To[%s] Key[%s] Succ!"
            //, pstMsgData->cMsgType == REQ_MSG ? "REQ" : "REP"
            //, pstMsgData->sSvrId
            , pcQSvrId, pstMsgData->sKey);
    cJSON_Delete(pstJson);
    return 0;
}
...
int tSvcACallEx(char *pcQSvrId, char *pcPSvrId, char *pcKey, cJSON *pstDataJson) {
    MsgData stMsgData;
    MEMSET_ST(stMsgData);
    tStrCpy(stMsgData.sSvrId, pcPSvrId, strlen(pcPSvrId));
    tStrCpy(stMsgData.sKey, pcKey, strlen(pcKey));
    stMsgData.pstDataJson = pstDataJson;
    return tSvcACall(pcQSvrId, &stMsgData);
}
...
int tReplyEx(char *pcQSvrId, char *pcPSvrId, char *pcKey, cJSON *pstDataJson) {
    ...
    tStrCpy(stMsgData.sKey, pcKey, strlen(pcKey));
    stMsgData.pstDataJson = pstDataJson;
    return tReply(pcQSvrId, &stMsgData);
}
...
int tWaitMsg(char *pcSvrId, MsgData *pstMsgData, int iTimeout) {
    ...
    tLog(WARN, "Recv Msg From[%s] Key[%s] Succ!"
...
int tDelKey(char *pcKey) {
    ...
    tLog(WARN, "Cmd[DEL] Key[%s] RTN[%d].", pcKey, pcRedisReply->integer);
    freeReplyObject(pcRedisReply);
