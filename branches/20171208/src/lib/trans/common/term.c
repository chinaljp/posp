...
int FindTermInfoById(Terminal *pcTerminal, char *pcMerchId, char *pcTermId) {
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "select a.STATUS,a.tmk,dt.SN from B_TERM a "
            " join B_MERCH b on a.USER_CODE = b.USER_CODE "
            " join b_dev_term dt on dt.term_id=a.term_id "
            " where A.TERM_ID='%s' AND B.MERCH_ID='%s' ", pcTermId, pcMerchId);
    ...
    while (OCI_FetchNext(pstRes)) {
        ...
        STRV(pstRes, 3, pcTerminal->sTermSn);
    }
