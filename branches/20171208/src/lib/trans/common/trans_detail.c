...
",MERCH_ORDER_NO,O_RRN,CARD_NO_ENCRYPT,MOBILE_ENCRYPT,CERT_NO_ENCRYPT,KEY_NAME"
...
int UpdPosTransLs(cJSON *pstJson) {
    ...
    char sRespCode[RESP_CODE_LEN + 1] = {0}, sIstRespCode[RESP_CODE_LEN + 1] = {0}, sRespLog[RESP_LOG_LEN + 1] = {0}, sRespDesc[RESP_LOG_LEN + 1] = {0};
    ...
    char sMcc[MCC_LEN + 1] = {0}, sUserCode[15 + 1] = {0}, sFeeType[2 + 1] = {0};
    char sMerchName[255 + 1] = {0}, sChannelMerchName[255 + 1] = {0}, sBusiType[2 + 1] = {0}, sAgentOrn[255 + 1] = {0}, sTermSn[100 + 1] = {0};
    double dFee = 0.0;
    OCI_Resultset *pstRes = NULL;
    ...
    //GET_STR_KEY(pstJson, "channel_mcc", sChannelMcc);
    tStrCpy(sChannelMcc, sChannelMerchId + 7, 4);
    GET_STR_KEY(pstJson, "autho_flag", sAuthoFlag);
    if ('\0' == sAuthoFlag[0])
    {
        tStrCpy(sAuthoFlag, "0", 1);
    }
    GET_STR_KEY(pstJson, "batch_no", sBatchNo);
    GET_STR_KEY(pstJson, "auth_code", sAuthCode);
    GET_STR_KEY(pstJson, "agent_id", sAgentId);
    ...
    GET_STR_KEY(pstJson, "merch_p_name", sMerchName);
    GET_STR_KEY(pstJson, "channel_merch_name", sChannelMerchName);
    GET_STR_KEY(pstJson, "busi_type", sBusiType);
    if ('\0' == sBusiType[0])
    {
        tStrCpy(sBusiType, "00", 2);
    }
    GET_STR_KEY(pstJson, "agent_organization", sAgentOrn);
    if (sFeeType[0] == '\0')
    {
        sFeeType[0] = 'B';
    }
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "UPDATE B_POS_TRANS_DETAIL SET "
            " RESP_CODE='%s', ISTRESP_CODE='%s',RESP_DESC='%s', RESP_ID='%s',RESP_LOG='%s' "
            ...
            " ,AGENT_ID='%s',TRANS_TYPE='%s',USER_CODE='%s',FEE_TYPE='%s',CHANNEL_RRN='%s'"
            ", merch_pname='%s',channel_merch_name='%s',busi_type='%s',AGENT_ORGANIZATION='%s',TERM_SN='%s'"
            " WHERE RRN='%s' and TRANS_DATE='%s'"
            , sRespCode, sIstRespCode, sRespLog, sRespId, sRespDesc
            ...
            , sCardBin, sCardType, sIssId, sAgentId, sTransType, sUserCode, sFeeType, sChannelRrn
            , sMerchName, sChannelMerchName, sBusiType, sAgentOrn, sTermSn
            , sRrn, sTransDate);
    ...
    tReleaseRes(pstRes);
    tLog(DEBUG, "更新交易date[%s]rrn[%s]结果成功.", sTransDate, sRrn);
    return 0;
}

int UpdPosTransResultLs(cJSON *pstJson) {
    char sSqlStr[1024] = {0};
    char sRrn[RRN_LEN + 1] = {0}, sChannelRrn[12 + 1] = {0}, sHisRrn[RRN_LEN + 1] = {0}, sFeeDesc[FEE_DESC_LEN + 1] = {0}, sFeeFlag[FEE_FLAG_LEN + 1], sSettleDate[SETTLE_DATE_LEN + 1] = {0};
    char sRespCode[RESP_CODE_LEN + 1] = {0}, sIstRespCode[RESP_CODE_LEN + 1] = {0}, sRespLog[RESP_LOG_LEN + 1] = {0}, sRespDesc[RESP_LOG_LEN + 1] = {0};
    char sRespId[INST_ID_LEN + 1] = {0};
    char sTransDate[10 + 1] = {0}, sHisTransDate[TRANS_DATE_LEN] = {0}, sChannelId[INST_ID_LEN + 1] = {0}, sChannelMerchId[MERCH_ID_LEN + 1] = {0};\
    char sChannelTermId[TERM_ID_LEN + 1] = {0}, sBatchNo[BATCH_NO_LEN + 1] = {0}, sAuthCode[AUTH_CODE_LEN + 1] = {0};
    char sChannelTermSn[SN_LEN + 1] = {0}, sChannelMcc[MCC_LEN + 1] = {0}, sAuthoFlag[AUTHO_FLAG_LEN + 1] = {0}, sAgentId[INST_ID_LEN + 1] = {0};
    char sCardBin[10 + 1] = {0}, sCardType[1 + 1] = {0}, sIssId[8 + 1] = {0}, sTransType[1 + 1] = {0};
    char sMcc[MCC_LEN + 1] = {0}, sUserCode[15 + 1] = {0}, sFeeType[2 + 1] = {0};
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "UPDATE B_POS_TRANS_DETAIL SET "
            ...
            " ,AUTH_CODE='%s', RESP_LOG='%s',channel_rrn='%s'"
            " ,LAST_MOD_TIME=sysdate"
            " WHERE RRN='%s' and trans_date='%s'"
            , sRespCode, sIstRespCode, sRespLog, sRespId, sAuthCode, sRespDesc, sChannelRrn
            , sRrn, sTransDate);
...
int AddInlineTransLs(cJSON *pstJson) {
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "INSERT INTO B_INLINE_TARNS_DETAIL(%s) "
            ...
            , sCardNoEnc, sMobileEnc, sCertNoEnc, sKeyName);
    tLog(DEBUG, "sql[%s]", sSqlStr);
    if (tExecute(&pstRes, sSqlStr) < 0)
    {
        tLog(ERROR, "添加交易rrn[%s]记录失败.", sRrn);
...
int UpdInlineTransLs(cJSON *pstJson) {
    ...
    */    
    if( sTransCode[3] == '1' || sTransCode[3] == '2'  ) {
        ...
        snprintf(sSqlStr, sizeof (sSqlStr), "UPDATE B_INLINE_TARNS_DETAIL SET "
                ...
                " ,LAST_MOD_TIME=sysdate"
                " ,AGENT_ID='%s',USER_CODE='%s',FEE_TYPE='%s'"
                " WHERE RRN='%s' and TRANS_DATE='%s'"
                , sRespCode, sRespLog, sRespId
                , sOrderUrl, sMerchOrderNo
                , dFee, sFeeDesc, sSettleDate, sFeeFlag, sChannelMerchId, sAgentId, sUserCode, sFeeType
                , sRrn, sTransDate);
    }
