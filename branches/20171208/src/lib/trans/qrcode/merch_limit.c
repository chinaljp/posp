...
int ChkMerchWxLimit(cJSON *pstTransJson, int *piFlag) {
    ...
    if (!DBL_ZERO(stLimitRule.dWXLimit) && DBL_CMP(dTranAmt, stLimitRule.dWXLimit)) {
        ErrHanding(pstTransJson, "C3", "商户[%s]交易[%s]单笔金额超限! 单笔限额[%f], 当前交易金额[%f].", sMerchId, sTransCode, stLimitRule.dWXLimit, dTranAmt);
        return -1;
    }
    //单日消费限额
...
int ChkMerchBbLimit(cJSON *pstTransJson, int *piFlag) {
    ...
    if (!DBL_ZERO(stLimitRule.dBBLimit) && DBL_CMP(dTranAmt, stLimitRule.dBBLimit)) {
        ErrHanding(pstTransJson, "C3", "商户[%s]交易[%s]单笔金额超限! 单笔限额[%f], 当前交易金额[%f].", sMerchId, sTransCode, stLimitRule.dBBLimit, dTranAmt);
        return -1;
    }
    //单日消费限额
...
int ChkMerchCupsLimit(cJSON *pstTransJson, int *piFlag) {
    ...
    if (!DBL_ZERO(stLimitRule.dCPLimit) && DBL_CMP(dTranAmt, stLimitRule.dCPLimit)) {
        ErrHanding(pstTransJson, "C3", "商户[%s]交易[%s]单笔金额超限! 单笔限额[%f], 当前交易金额[%f].", sMerchId, sTransCode, stLimitRule.dCPLimit, dTranAmt);
        return -1;
    }
    //单日消费限额
