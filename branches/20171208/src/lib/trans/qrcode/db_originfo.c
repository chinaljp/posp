...
int FindOriTransMsg(char *pcOrderNo, char *pcTransCode, char *pcChannelMerchId,char *pcValidFlag, double *pdAmount, double *pdFee, char *pcSettleDate, char *pcOrrn) {
//int FindOriTransMsg(char *pcOrderNo, char *pcTransCode, char *pcChannelMerchId,char *pcValidFlag) {
    OCI_Resultset *pstRes = NULL;
    double dAmount = 0.00, dFee = 0.00;
    char sSqlStr[1024];
    MEMSET(sSqlStr);
    
    tTrim(pcOrderNo);
    snprintf(sSqlStr,sizeof(sSqlStr),"select trans_code,channel_merch_id,valid_flag,amount,fee,settle_date,rrn "
    ...
    if (NULL == pstRes) {
     ...
     while (OCI_FetchNext(pstRes)) {
        ...
        DOUV(pstRes, 5, dFee);
        STRV(pstRes, 6, pcSettleDate);
        STRV(pstRes, 7, pcOrrn);
