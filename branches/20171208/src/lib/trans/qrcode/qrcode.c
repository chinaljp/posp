...
/* 主扫模式 */
int QrcodeActive(cJSON *pstTransJson, int *piFlag) {
    /*   扫描付款码二维码获取到的用户身份字符串  */
...
/*二维码下单交易*/
int QrcodeOrder(cJSON *pstTransJson, int *piFlag) {
    ...
    tLog(DEBUG, "qr_order_no[%s]", sOrder_no);
    /* 请求交易流中将订单号更新到本次交易的流水记录中(二维码主扫 被扫,撤销)*/
...
int RespInfo(cJSON *pstJson, int *piFlag) {
    ...
    SET_STR_KEY(pstJson, "code_url", sCodeUrl);
...
/*****************************************************************************************/
int UpOrderValidProc(cJSON *pstDataJson, int *piFlag) {
    int iRet = 0;
...
/**********************************************************************************************/
int UpInLineValidProc(cJSON *pstDataJson, int *piFlag) {
    int iRet = 0;
...
/*更新消费撤销原交易*/
int UpdQrTransDetail(cJSON *pstTransJson, int *piFlag) {
...
int QueryReturnInfo(cJSON *pstJson, int *piFlag) {
   ...
   //char sSettleKey[38 + 1] = {0};
    ...
    SET_STR_KEY(pstJson, "settleKey", sSettleKey);
    
    GET_STR_KEY(pstRepJson, "cardAttr", sCardAttr);
    SET_STR_KEY(pstJson, "cardAttr", sCardAttr);
...
int RetFalse(cJSON *pstTransJson, int *piFlag) {
    tLog(INFO, "暂不支持此交易");
    ErrHanding(pstTransJson, "FA", "暂不支持此交易");
    return (-1);
}

int PosVersionCtrl(cJSON *pstDataJson, int *piFlag) {
    cJSON *pstRepJson = NULL;
