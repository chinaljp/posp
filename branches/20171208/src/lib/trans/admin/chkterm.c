...
int ChkTerm(cJSON *pstTransJson, int *piFlag) {
    char sMerchId[MERCH_ID_LEN + 1] = {0}, sTermId[TERM_ID_LEN + 1] = {0};
    Terminal stTerminal;
    cJSON *pstTermJson = NULL;
    ...
    SET_JSON_KEY(pstTransJson, "terminal", pstTermJson);
    tLog(INFO, "商户[%s]终端[%s]检查成功.", sMerchId, sTermId);
    return 0;
}