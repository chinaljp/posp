...
int GetQICParam(char *pcQICParam, int *piLength, char * pcMerchId) {
    char sFF805D[2] = {0};
    char sFF803A[4] = {0};
    char sFF803C[4] = {0};
    char sFF8058[13] = {0};
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "SELECT FF805D, FF803A, FF803C, FF8058,FF8054, FF8055,  \
     ...
     where USER_CODE in((SELECT USER_CODE FROM B_MERCH WHERE MERCH_ID = '%s'),'AAAAAAAAAAAAAAA') \
     order by user_code desc ", pcMerchId);
...
int UpQicFlag(char * pcMerchId) {
    char sSqlStr[256], sSqlStr1[256];
    int iRet = 0;
    OCI_Resultset *pstRes = NULL;

    snprintf(sSqlStr, sizeof (sSqlStr), "SELECT COUNT(1) FROM b_term_qic_param \
    ...
    if (iRet == 1)
    {
        snprintf(sSqlStr1, sizeof (sSqlStr1), "UPDATE B_MERCH_DOUBLE_FREE SET STATUS='1' "
                " where USER_CODE = (SELECT USER_CODE FROM B_MERCH WHERE MERCH_ID = '%s')", pcMerchId);

        if (tExecute(&pstRes, sSqlStr1) < 0 || tGetAffectedRows() <= 0)
        {
            tLog(ERROR, "更新[%s]下载参数标志失败.", pcMerchId);
            return -1;
        }
        tReleaseRes(pstRes);
        tLog(INFO, "更新商户[%s]参数标志成功,商户已开通双免", pcMerchId);
        return 0;
    } else if (iRet < 0)
    {
        tLog(ERROR, "未找到记录.");
...
/* 非接参数 */
int DownQICParam(cJSON *pstTransJson, int *piFlag) {
    char sTmp[1024] = {0}, sMerchId[15 + 1] = {0};
    int iLen = 0;

    GET_STR_KEY(pstTransJson, "merch_id", sMerchId);
    if (GetQICParam(sTmp, &iLen, sMerchId) < 0)
    {
...
int UPQicParamFlag(cJSON *pstTransJson, int *piFlag) {
    char sTmp[1024] = {0}, sMerchId[15 + 1] = {0};
    int iLen = 0;

    GET_STR_KEY(pstTransJson, "merch_id", sMerchId);
    if (UpQicFlag(sMerchId) < 0)
    {
        ErrHanding(pstTransJson, "96", "非接参数下载失败.");
        return -1;
    }
...
/* 免密新增卡bin下载          */
int NoPwBin(cJSON *pstTransJson, int *piFlag) {
...
/* 免密卡bin黑名单下载 */
int BinBkList(cJSON *pstTransJson, int *piFlag) {
...
/* IC卡公钥下载 */
int DownPubKey(cJSON *pstTransJson, int *piFlag) {
    char sData[200] = {0};
...
/* IC卡参数下载 */
int DownICParam(cJSON *pstTransJson, int *piFlag) {
    char sData[200] = {0};
    char sBuf[2048] = {0}, sTmp[1024];
    int iLen = 0;
