...
int ChkMerchNosign(char * pcMerchId) {
    char sSqlStr[512];
    int iNum = 0;
    OCI_Resultset *pstRes = NULL;

    snprintf(sSqlStr, sizeof (sSqlStr), "SELECT COUNT(1) FROM B_MERCH_DOUBLE_FREE \
      WHERE USER_CODE = ( SELECT USER_CODE FROM B_MERCH WHERE MERCH_ID='%s') AND STATUS='1'", pcMerchId);
...
int CalcMerchFee(cJSON *pstTransJson, int *piFlag) {
    int iRet = -1;
    iRet = ChkMerchFeeType(pstTransJson, piFlag);
    if (iRet < 0)
        return iRet;
    iRet = MerchCardFee(pstTransJson, piFlag);
    return iRet;
}

/* 判断手续费类型 */
int ChkMerchFeeType(cJSON *pstTransJson, int *piFlag) {
...
/* 计算银行卡手续费 */
int MerchCardFee(cJSON *pstTransJson, int *piFlag) {
