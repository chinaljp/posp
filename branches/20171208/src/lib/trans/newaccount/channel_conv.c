...
int ConvLifeMerch(cJSON *pstTransJson, int *piFlag) {
    ...
    char sChannelMerchName[256 + 1] = {0},sBusiType[2 + 1]={0};
    int iRet = -1;
    ...
    GET_STR_KEY(pstTransJson, "busi_type",sBusiType);
    ...
    //busi_type业务类型:00:正常,01:生活圈,02:优惠商户
    SET_STR_KEY(pstTransJson, "channel_id", "49000000");
    if (FindLifeMerch(sChanneMerchId, sChannelTermId, sChannelMerchName, sUserCode, sAgentId) < 0)
    {
        ...
        SET_STR_KEY(pstTransJson, "busi_type", "00");
    ...
    } else
    {
        if (sChanneMerchId[0] != '\0' && sChannelTermId[0] != '\0')
        {
            ...
            SET_STR_KEY(pstTransJson, "busi_type", "01");
...
int ChkChannelMerch(cJSON *pstJson, char * pcChanneMerchId) {
    ...
    if (stPufaLimitRule.sCardType[0] != '2')
    {/* 当标准为2时不限借贷记 */
...
int ConvChannelMerch(cJSON *pstTransJson, int *piFlag) {
    ...
    } else
    {
        ...
        SET_STR_KEY(pstTransJson, "busi_type", "02");
        tLog(INFO, "优惠类渠道商户[%s:%s:%s].", sChanneMerchId, sChannelTermId, sChannelId);
    }
    return 0;
}

int UnChannelLimit(cJSON *pstTransJson, int *piFlag) {

    char sMerchId[15 + 1] = {0};
    double dTranAmt = 0.0L;
    char sTransCode[9] = {0}, sRespCode[2 + 1] = {0};
    char sCardType[1 + 1] = {0}, sBusiType[10 + 1] = {0};
    ...
    if (memcmp(sBusiType, "02", 2))
    {
        tLog(INFO, "交易非优惠路由业务，忽略.");
        return 0;
    }
    dTranAmt /= 100;
    if (!memcmp(sRespCode, "00", 2))
    {
        tLog(DEBUG, "商户[%s]交易成功,无需恢复累计额[%f].", sMerchId, dTranAmt);
        return 0;
    }

    /* 检查是否有控制 */
    MEMSET_ST(stTransCode);
    FindTransCode(&stTransCode, sTransCode);
    if (stTransCode.sSettleFlag[0] == '0')
    {
        tLog(INFO, "非账务类交易[%s],不检查限额,忽略", sTransCode);
        return 0;
    }
    if ('+' == stTransCode.sAccFlag[0])
    {
        dTranAmt = -dTranAmt;
    }
    ...
    {
        tLog(INFO, "未找到渠道商户[%s]的限额规则,忽略.", sChanneMerchId);
        return 0;
    }
    ...
    {
        tLog(INFO, "未找到本地商户[%s]的限额规则,忽略.", sMerchId);
        return 0;
    }
    ...
    {
        ErrHanding(pstTransJson, sRespCode, "渠道商户[%s]回复累计额[%f]失败.", sChanneMerchId, dTranAmt);
        return 0;
    }
    /* 机构类商户更新本地商户限额 */
    if (stPufaLimitRule.sMerchType[0] == '1')
    {
        ...
        {
            ErrHanding(pstTransJson, sRespCode, "商户[%s]恢复累计额[%f]失败.", sMerchId, dTranAmt);
            return 0;
        }
    }

    tLog(INFO, "商户[%s]恢复累计额成功,交易金额[%f]", sMerchId, dTranAmt);
    return 0;

}

int UpChannelLimit(cJSON *pstTransJson, int *piFlag) {

    char sChanneMerchId[15 + 1] = {0}, sChanneTermId[8 + 1] = {0}, sChannelId[8 + 1] = {0};
    char sMerchId[15 + 1] = {0}, sTraceNo[6 + 1] = {0}, sTransTime[6 + 1] = {0}, sTransCode[6 + 1] = {0}, sBusiType[10 + 1] = {0};
    double dTranAmt = 0.0L;
    PufaLimitRule stPufaLimitRule;
    TransCode stTransCode;
...
/* 非生活圈交易转换银联商户号 */
int ConvCupsMerch(cJSON *pstTransJson, int *piFlag) {
    char sMerchId[MERCH_ID_LEN + 1] = {0}, sTermId[TERM_ID_LEN + 1] = {0}, sMerchPName[200 + 1] = {0}, sTermSn[50 + 1] = {0};

    GET_STR_KEY(pstTransJson, "merch_id", sMerchId);
    GET_STR_KEY(pstTransJson, "term_id", sTermId);
    GET_STR_KEY(pstTransJson, "term_sn", sTermSn);
    GET_STR_KEY(pstTransJson, "merch_p_name", sMerchPName);

    SET_STR_KEY(pstTransJson, "channel_merch_id", sMerchId);
    SET_STR_KEY(pstTransJson, "channel_term_id", sTermId);
    SET_STR_KEY(pstTransJson, "channel_merch_name", sMerchPName);
    SET_STR_KEY(pstTransJson, "channel_id", "49000000");
    SET_STR_KEY(pstTransJson, "channel_term_sn", sTermSn);
    tLog(INFO, "使用本地商户[%s:%s].", sMerchId, sTermId);
    return 0;
}
...
/* 反向交易需要使用正向交易的上送商户号 */
int GetMerchName(cJSON *pstTransJson, int *piFlag) {
    char sMerchId[15 + 1] = {0}, sChannelMerchId[15 + 1] = {0};
