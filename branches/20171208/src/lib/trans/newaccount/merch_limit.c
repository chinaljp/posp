...
/******************************************************************************/
int ChkMerchCardLimit(cJSON *pstTransJson, int *piFlag) {
...
/******************************************************************************/
int UnMerchCardLimit(cJSON *pstTransJson, int *piFlag) {
    ...
    {
        tLog(DEBUG, "商户[%s]交易成功,无需恢复累计额[%f],忽略.", sMerchId, dTranAmt);
        return 0;
    }
    /* 检查是否有控制 */
    FindTransCode(&stTransCode, sTransCode);
