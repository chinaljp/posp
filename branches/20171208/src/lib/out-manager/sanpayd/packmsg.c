...
int SetFld(int iBitNo, cJSON *pstQrcode, cJSON *pstOutJson) {
    ...
    switch (iBitNo) {
        ...
        case 2: /* 付款金额(元) */
            ...
            GET_DOU_KEY(pstQrcode, "amount", dAmount);
            snprintf(sAmount, sizeof (sAmount), "%.2lf", dAmount / 100);
            tTrim(sAmount);
            SET_STR_KEY(pstOutJson, "amount", sAmount);
            return iBitNo;
        case 3: /* 二维码有效时间 */
            GET_STR_KEY(pstQrcode, "expire", sBuf);
            SET_STR_KEY(pstOutJson, "expire", sBuf);
            return iBitNo;
        case 4: /* 付款码 */
        ...
        case 20: /*订单时间 YYYYMMDDHHMMSS*/
            ...
            return iBitNo;
        case 21: /*商户号*/
            GET_STR_KEY(pstQrcode, "merch_id", sBuf);
        ...
        case 22: /*商户名称*/
            ...
            return iBitNo;
        case 23: /*原交易订单时间 YYYYMMDDHHMMSS*/
            GET_STR_KEY(pstQrcode, "trans_date", sBuf);
            GET_STR_KEY(pstQrcode, "o_rrn", sBuf1);
            sBuf1[6] = 0;
            strcat(sBuf, sBuf1);  //取原交易时间，原交易o_rrn的前六位
        ...
        case 26: /* 付款金额(分) */
            //GET_INT_KEY(pstQrcode, "amount", iAmount);
            GET_DOU_KEY(pstQrcode, "amount", dAmount);
            snprintf(sAmount, sizeof (sAmount), "%.f", dAmount);
            tTrim(sAmount);
            SET_STR_KEY(pstOutJson, "cent", sAmount);
            return iBitNo;
...
int RequestMsg(cJSON *pstReqJson, cJSON *pstDataJson) {
    ...
    if (0 == memcmp(sTransCode + 2, "W100", 4) || 0 == memcmp(sTransCode + 2, "B100", 4)) {
        SET_STR_KEY(pstDataJson, "op_type", TYPE_A);
        tLog(INFO, "订单号[%s]主扫模式", sOrderNo);
    } else if (0 == memcmp(sTransCode + 2, "W200", 4) || 0 == memcmp(sTransCode + 2, "B200", 4)) {
        SET_STR_KEY(pstDataJson, "op_type", TYPE_P);
        tLog(INFO, "订单号[%s]被扫模式", sOrderNo);
    } else if (0 == memcmp(sTransCode + 2, "W300", 4) || 0 == memcmp(sTransCode + 2, "B300", 4)) {
        SET_STR_KEY(pstDataJson, "op_type", TYPE_Q);
        tLog(INFO, "订单号[%s]查询", sOrderNo);
    } else if (0 == memcmp(sTransCode + 2, "W600", 4) || 0 == memcmp(sTransCode + 2, "B600", 4)) {
