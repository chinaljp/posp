...
static char g_sConfig[64+1]; /* 配置文件 */
...
int InitParam(int iArgc, char* pcArgv[]) {
    ...
    if (g_sAppNum[0] == '\0' || g_sLibName[0] == '\0' ||g_sConfig[0] == '\0'
            || g_sLogName[0] == '\0' || g_sSvrName[0] == '\0') {
        fprintf(stderr, "Basic Usage: %s -f[Config Name] -l [Log Name] -s [Svr Name] -b[Lib Name]-n [App Num]\n", pcArgv[0]);
