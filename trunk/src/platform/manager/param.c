...
int InitParam(int iArgc, char* pcArgv[]) {
    ...
    while ((iOpt = getopt(iArgc, pcArgv, "hH:l:L:b:B:s:S:n:N:")) != -1)
        switch (iOpt) {
    ...
    if (g_sAppNum[0] == '\0' || g_sLibName[0] == '\0'
        ...
        fprintf(stderr, "Basic Usage: %s  -l [Log Name] -s [Svr Name]  -n [App Num] -b[Lib Name]\n", pcArgv[0]);
        return -1;
    }
    fprintf(stderr, "%s  -s %s -n %s -l %s -b %s 成功\n", pcArgv[0], g_sSvrName, g_sAppNum, g_sLogName, g_sLibName);
    return 0;
}