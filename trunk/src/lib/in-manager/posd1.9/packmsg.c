...
2, 11, 12, 13, 14, 23, 25, 27, 32, 37, 39, 41, 42, 44, 49, 53, 54, 55, 60, 63 ,64, 255};
...
2, 4, 11, 12, 13, 14, 15, 23, 25, 27, 28, 29, 30, 32, 37, 38, 39, 41, 42, 44, 49, 53, 54, 55,59, 60, 63, 64, 255};
/* POS脱机消费 */
...
2, 4, 11, 12, 13, 14, 15, 23, 25, 28, 29, 30, 32, 37, 38, 39, 41, 42, 44, 49, 53, 55, 60, 63, 64, 255};
...
2, 4, 11, 12, 13, 14, 15, 23, 25, 27, 28, 29, 30, 32, 37, 38, 39, 41, 42, 44, 49, 53, 55, 59, 60, 63, 64, 255};

/* POS退货 */
static unsigned char g_caRefundUnpackMap[] = {\
2, 4, 11, 14, 22, 23, 25, 26, 32, 37, 38, 41, 42, 35, 36, 49, 52, 53, 55, 60, 61, 63, 64, 255};
static unsigned char g_caRefundPackMap[] = {\
2, 4, 11, 12, 13, 14, 15, 23, 25, 27, 28, 29, 30, 32, 37, 39, 41, 42, 44, 49, 53, 55, 59, 60, 63, 64, 255};
...
2, 4, 11, 12, 13, 14, 15, 23, 25, 27, 32, 37, 38, 39, 41, 42, 44, 49, 53, 54, 55, 59, 60, 63, 64, 255};
...
2, 4, 11, 12, 13, 14, 15, 23, 25, 27,  32, 37, 38, 39, 41, 42, 44, 49, 53, 55, 59, 60, 63, 64, 255};
...
7, 11, 41, 42, 60, 64, 255};
static unsigned char g_caOrderPackMap[] = {\
11, 12, 13, 37, 39, 41, 42, 44, 54, 60, 64, 255};


/* 订单消费 */
static unsigned char g_caorderPurchaseUnpackMap[] = {\
2, 4, 7, 11, 14, 22, 23, 25, 26, 32 ,41, 42, 35, 36, 49, 52, 53, 55, 57, 59, 60, 63, 64, 255};
static unsigned char g_caorderPurchasePackMap[] = {\
2, 4, 11, 12, 13, 14, 15, 23, 25, 27, 28, 29, 30, 32, 37, 38, 39, 41, 42, 44, 49, 53, 54, 55,59, 60,63, 64, 255};

/* 订单消费冲正 */
static unsigned char g_caorderPurchaseRevUnpackMap[] = {\
2, 4, 7, 11, 14, 22, 23, 25, 32, 38, 39, 41, 42, 49, 53, 55, 60,62, 64, 255};
...
2, 4, 7, 11, 14, 22, 23, 25, 26, 32, 37, 38, 41, 42, 35, 36, 49, 52, 53, 55, 60, 61, 62, 63, 64, 255};
...
2, 4, 7, 11, 14, 22, 23, 25, 32, 38, 39, 41, 42, 49, 53, 55, 60, 61, 62,64, 255};
...
int SetFld(int iBitNo, cJSON *pstJson) {
    ...
    char sTmkFlag[1 + 1] = {0}, sCardType[2 + 1] = {0}, sChannelId[8 + 1] = {0}, sLifeType[3 + 1] = {0};
    double dAmount = 0.0;
    int iLen, iRet;
    ...
    switch (iBitNo) {
        ...
        case 25: /* 服务点条件码 */
            ...
            return ( tIsoSetBit(&g_stIsoData, iBitNo, sBuf, strlen(sBuf)));
        case 27: /* 卡类型 */
            GET_STR_KEY(pstJson, "card_type", sBuf);
            if (sBuf[0] == '0') {
                sCardType[0] = '0';
            } else if (sBuf[0] == '1') {
                sCardType[0] = '1';
            }
            tLog(INFO, "sCardType=%c", sCardType[0]);
            return ( tIsoSetBit(&g_stIsoData, iBitNo, sCardType, strlen(sCardType)));
        case 28: /* 打印终端编号 */
            /*跳转优惠类渠道的不返回渠道商户终端号*/
            GET_STR_KEY(pstJson, "channel_id", sChannelId);
            if (!memcmp(sChannelId, "48560000", 8) || !memcmp(sChannelId, "49000000", 8)) {
                GET_STR_KEY(pstJson, "life_type", sLifeType);
                if (!strcmp(sLifeType, "001")) {
                    GET_STR_KEY(pstJson, "life_channel_term_id", sBuf);
                    return ( tIsoSetBit(&g_stIsoData, iBitNo, sBuf, strlen(sBuf)));
                }
                return 0;
            }
        ...
        case 29: /* 打印商户编号 */
            ...
            if (!memcmp(sChannelId, "48560000", 8) || !memcmp(sChannelId, "49000000", 8)) {
                GET_STR_KEY(pstJson, "life_type", sLifeType);
                if (!strcmp(sLifeType, "001")) {
                    GET_STR_KEY(pstJson, "life_channel_merch_id", sBuf);
                    return ( tIsoSetBit(&g_stIsoData, iBitNo, sBuf, strlen(sBuf)));
                }
                return 0;
            }
        ...
        case 30: /* 打印商户名称 */
            ...
            if (!memcmp(sChannelId, "48560000", 8) || !memcmp(sChannelId, "49000000", 8)) {
                GET_STR_KEY(pstJson, "life_type", sLifeType);
                if (!strcmp(sLifeType, "001")) {
                    GET_STR_KEY(pstJson, "life_channel_merch_name", sBuf);
                    return ( tIsoSetBit(&g_stIsoData, iBitNo, sBuf, strlen(sBuf)));
                }
                return 0;
            }
...
int RequestMsg(cJSON *pstReqJson, cJSON *pstDataJson) {
    ...
    char sTransCode[TRANS_CODE_LEN + 1] = {0}, sAcqInstId[INST_ID_LEN + 1] = {0}, sBatchNo[BATCH_NO_LEN + 1] = {0}, sTpdu[10 + 1] = {0};
