...
int FindCardName(char * pstMerchid, char *pstName, char *pcIdCard) {
    ...
    if (tHsm_Dec_Asc_Data(sDName, sKey, sName) < 0) {
        tLog(ERROR, "解密结算人姓名数据失败.");
        tReleaseRes(pstRes);
        return -1;
    }

    if (tHsm_Dec_Asc_Data(sIDCardD, sKey, sIDCard) < 0) {
        tLog(ERROR, "解密结算人身份证号数据失败.");
        tReleaseRes(pstRes);
        return -1;
    }

    tLog(ERROR, "上送姓名[%s]", sPrivateData);
    tLog(ERROR, "解密姓名[%s]", sDName);

    tLog(ERROR, "上送身份证号[%s]", pcIdCard);
    tLog(ERROR, "解密身份证号[%s]", sIDCardD);
    ...
    if (i >= 1) {
        if (i == 1) {
            ...
            if (FindValueByKey(sKey, "MERCH_AUTH_TOTAL_LIMIT_ONCE") < 0) {
                tLog(ERROR, "查找key[MERCH_AUTH_TOTAL_LIMIT_ONCE]值,失败.");
                return -1;
            }
            dTotalLimit = atof(sKey);
            /*查找首次借记卡鉴权额度*/
            if (FindValueByKey(sKey, "MERCH_AUTH_DEBIT_LIMIT_ONCE") < 0) {
                tLog(ERROR, "查找key[MERCH_AUTH_DEBIT_LIMIT_ONCE]值,失败.");
                return -1;
            }
            dDebitLimit = atof(sKey);
            /*查找首次EPOS鉴权额度*/
            if (FindValueByKey(sKey, "MERCH_AUTH_EPOS_TOTAL_LIMIT_ONCE") < 0) {
                tLog(ERROR, "查找key[MERCH_AUTH_EPOS_TOTAL_LIMIT_ONCE]值,失败.");
                return -1;
            }
            ...
            if (FindValueByKey(sKey, "MERCH_AUTH_TOTAL_LIMIT_TWICE") < 0) {
                tLog(ERROR, "查找key[MERCH_AUTH_TOTAL_LIMIT_TWICE]值,失败.");
                return -1;
            }
            dTotalLimit = atof(sKey);
            /*查找二次EPOS鉴权额度*/
            if (FindValueByKey(sKey, "MERCH_AUTH_EPOS_TOTAL_LIMIT_TWICE") < 0) {
                tLog(ERROR, "查找key[MERCH_AUTH_EPOS_TOTAL_LIMIT_TWICE]值,失败.");
                return -1;
            }
            dEposLimit = atof(sKey);
