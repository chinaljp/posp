...
int FindMerchD0Type(char *pcD0Type, char *pcUserCode){
    char sSqlStr[512] = {0};
    OCI_Resultset *pstRes = NULL;
    tTrim(pcUserCode);

    snprintf(sSqlStr, sizeof (sSqlStr), "SELECT D0_Type FROM b_merch_fee  \
...
int FindT0Merchlimit(T0Merchlimit * pstMerchlimit, char * pcUserCode) {
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "SELECT USER_CODE, TOTAL_LIMIT, USED_LIMIT, USABLE_LIMIT, DEBIT_TOTAL_LIMIT,DEBIT_PAID_AMT,DEBIT_AVL_AMT FROM B_MERCH_AUTH_LIMIT  \
WHERE USER_CODE = '%s' FOR UPDATE", sUserCode);
    ...
    while (OCI_FetchNext(pstRes)) {
        ...
        DOUV(pstRes, 5, st0Merchlimit.dDTotallimit);
        DOUV(pstRes, 6, st0Merchlimit.dDPaidAmt);
        DOUV(pstRes, 7, st0Merchlimit.dDAvlAmt);
    }
 ...
 **
 *****************************************************************************/
int UptT0Limit(double dAmount, char *pcUserCode, char *pcCardType) {
    char sSqlStr[512] = {0};
    OCI_Resultset *pstRes = NULL;
    if (pcCardType[0] == '0') {
        snprintf(sSqlStr, sizeof (sSqlStr), "UPDATE  B_MERCH_AUTH_LIMIT SET USED_LIMIT = USED_LIMIT+%f "
                ",USABLE_LIMIT = USABLE_LIMIT-%f, DEBIT_AVL_AMT = DEBIT_AVL_AMT-%f, DEBIT_PAID_AMT = DEBIT_PAID_AMT +%f \
         WHERE USER_CODE = '%s'", dAmount, dAmount, dAmount, dAmount, pcUserCode );
    } else {
        snprintf(sSqlStr, sizeof (sSqlStr), "UPDATE  B_MERCH_AUTH_LIMIT SET USED_LIMIT = USED_LIMIT+%f "
