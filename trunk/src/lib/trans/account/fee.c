...
int IsSaleMcc(char *pcMcc) {
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "select consume_type from s_mcc where mcc= :mcc");

    if (tExecutePre(&pstRes, sSqlStr, 0, 0, 1, pcMcc,":mcc") < 0) {
...
int CalcMerchFee2Free(cJSON *pstTransJson) {
    char sMerchId[MERCH_ID_LEN + 1] = {0}, sTermId[TERM_ID_LEN + 1] = {0}, sCardType[2] = {0}, sMcc[4 + 1] = {0};
...
#endif 
    if (ChkMerchNosign(sMerchId, sTermId) != 0) {
        tLog(ERROR, "商户未开通双免");
        return 0;
    }
