...
int AddD0PosWallet(cJSON *pstJson, int *piFlag) {
    ...
    stQMsgData.pstDataJson = pstNetJson;
    iRet = tSvcCall("00T700_Q", &stQMsgData, stQMsgData.sSvrId, &stPMsgData, 20);
...
int AddAppPosWallet(cJSON *pstJson, int *piFlag) {
    ...
    stQMsgData.pstDataJson = pstNetJson;
    iRet = tSvcCall("00T900_Q", &stQMsgData, stQMsgData.sSvrId, &stPMsgData, 20);
...
int ChkWalletType(cJSON *pstTranData, int *piFlag) {
    ...
    char sUserCode[15 + 1] = {0},sD0Type[2 + 1] = {0};
     ...
     *  */
     ...
     /*BENGIN 特殊D0开通标志， 如果已开通， IC 借记卡 入DO 钱包  trans_type类型:0:D0, add by gjq at 20180622*/
    GET_STR_KEY(pstTransJson, "user_code", sUserCode);
    if (FindMerchD0Type(sD0Type, sUserCode) < 0 ) {
        ErrHanding(pstTransJson, "96", "检索不到该商户[%s]开通特殊D0标志", sMerchId);
        return -1;
    }
     /* END */
    ...
    if ((sCardType[0] == '1')&&(sInputMode[1] == '2')) {
        ...
        if( sInputMode[1] == '5' || sInputMode[1] == '7') {
            tLog(WARN,"商户[%s]本次交易为IC芯片借记卡",sMerchId);
            if( sD0Type[0] == '1' ) {
                tLog(WARN,"商户[%s]D0类型[%s]:已开通特殊d0",sMerchId,sD0Type);
                sTransType[0] = '0';
            }
            else {
                tLog(WARN,"商户[%s]D0类型[%s]:未开通特殊d0",sMerchId,sD0Type);
                sTransType[0] = '3';
            }
        }
        else {
...
int FrozenWallet(cJSON *pstJson, int *piFlag) {
    ...
    stQMsgData.pstDataJson = pstNetJson;
    iRet = tSvcCall("00T400_Q", &stQMsgData, stQMsgData.sSvrId, &stPMsgData, 20);
    // iRet = tSvcCall("00T400_Q", pstNetJson, &pstRecvJson, 30);
    if (iRet < 0) {
        if (MSG_TIMEOUT == iRet) {
            /* Begin 钱包冻结通知超时请求交易流中发起解冻  add by GJQ at 20180309*/
            tLog( ERROR, "交易[%s]钱包冻结超时,解冻处理.", sRrn );
            DEL_KEY(pstNetJson,"trans_code");
            SET_STR_KEY( pstNetJson, "trans_code", "00T500" );
            tGetUniqueKey( stQMsgData.sSvrId );
            snprintf( stQMsgData.sKey, sizeof (stQMsgData.sKey ), "00T500%s", sRrn );
            stQMsgData.pstDataJson = pstNetJson;
            tSvcACall("00T500_Q", &stQMsgData);//异步发起解冻
            ErrHanding(pstTransJson, "96", "交易[%s]钱包冻结通知超时.", sRrn);
            /* End 钱包冻结通知超时请求交易流中发起解冻  add by GJQ at 20180309*/
        } else
...
int UnFrozenWallet(cJSON *pstJson, int *piFlag) {
    ...
    snprintf(stQMsgData.sKey, sizeof (stQMsgData.sKey), "00T500%s", sRrn);
    stQMsgData.pstDataJson = pstNetJson;
    iRet = tSvcCall("00T500_Q", &stQMsgData, stQMsgData.sSvrId, &stPMsgData, 20);
...
int QueryWallet(cJSON *pstJson, int *piFlag) {
    ...
    stQMsgData.pstDataJson = pstNetJson;
    iRet = tSvcCall("00AQ00_Q", &stQMsgData, stQMsgData.sSvrId, &stPMsgData, 20);
    if (iRet < 0) {
        if (MSG_TIMEOUT == iRet) {
