...
int FindCodeRule(RiskRule *pstRiskRule, char * pcRuleCode) {
    ...
    tLog(INFO, "RULECODE[%s]", sRuleCode);
    ...
    tLog(INFO, "风控信息:sRuleCode[%s],sRuleName[%s],sStatus[%s],sTransCode[%s],iTransCnt[%d],\
    ...
    *pstRiskRule = stRiskRule;
    tLog(INFO, "已找到分控规则码[%s]参数", sRuleCode);
    tReleaseRes(pstRes);
    return 0;
}
...
int RuleCodefunc1(cJSON * pstJson, RiskRule *stRiskRule) {
    ...
    tLog(INFO, "风控规则[R001]OR[R002]OR[R015]执行中...");
...
/* 7、同借记卡，同商户，1分钟内连续交易（消费，预授权完成）冻结账户 ROO7*/
int RuleCodefunc7(cJSON * pstJson, RiskRule *stRiskRule) {
    char sKey[128 + 1] = {0};
    char sMerchId[15 + 1] = {0}, sCardNo[19 + 1] = {0}, sTransCode[6 + 1] = {0};
    char sTransDate[8+1] = {0}, sTransTime[6 + 1] = {0}, sTransStamp[14+1] = {0}, sNtTransStamp[14+1] = {0};
    ...
    sprintf(sTransStamp,"%s%s",sTransDate,sTransTime);
    ...
    /*sTransStamp 前一分钟的时间点*/
    tAddTime(sTransStamp, -60, sNtTransStamp);
    tLog(DEBUG,"sTransStamp[%s], sNtTransStamp[%s]",sTransStamp,sNtTransStamp);
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "select count(1) from b_pos_trans_detail \
                    ...
                    and trans_date||trans_time between  '%s' and '%s' and resp_code='00'", sMerchId, sECardNo, sNtTransStamp, sTransStamp);
tLog(DEBUG, "SQL[%s]", sSqlStr);
    iRet = tQueueCnt(sSqlStr);
    if (iRet < 0) {
    ...
    if (iRet >= stRiskRule->iTransCnt) { /*连续交易 iTransCnt 次 第 iTransCnt+1次 交易（即：本次交易之前1分钟内已经连续交易 iTransCnt 次）时进行冻结*/
        /*冻结账户*/
        tLog(DEBUG, "借记卡[%s]在商户[%s]1分钟内交易笔次[%d],冻结账户!", sCardNo, sMerchId, iRet);
        iRet = RiskOpt(pstJson, stRiskRule);
        if (iRet < 0) {
            tLog(INFO, "风控规则[%s]操作动作执行失败!", stRiskRule->sRuleCode);
        } else if (iRet == 1) {
...
int RuleCodefunc12(cJSON * pstJson, RiskRule *stRiskRule) {
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "select count(1) from b_pos_trans_detail \
                    ...
                    and trans_date||trans_time between '%s' and '%s' and resp_code='00'", 
                                            sMerchId, sECardNo, sNtTransStamp, sTransStamp);
tLog(DEBUG, "SQL[%s]", sSqlStr);
    iRet = tQueueCnt(sSqlStr);
    if (iRet < 0) {
        tLog(DEBUG, "sSqlStr[%s]", sSqlStr);
...
int RuleCodefunc16(cJSON * pstJson, RiskRule *stRiskRule) {
    ...
    GET_STR_KEY(pstJson, "trans_date", sTransDate);
    sprintf(sTransStamp,"%s%s",sTransDate,sTransTime);
    /*sTransStamp 前一小时的时间点*/
    tAddTime(sTransStamp, -3600, sNtTransStamp);
    tLog(DEBUG,"sTransStamp[%s], sNtTransStamp[%s]",sTransStamp,sNtTransStamp);
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "select count(1) from b_pos_trans_detail \
                    ...
                    and trans_date||trans_time between '%s' and '%s' and istresp_code='%s'",
                            sMerchId, sECardNo, sNtTransStamp, sTransStamp, stRiskRule->sRespCode);
...
int RuleCodefunc20(cJSON * pstJson, RiskRule *stRiskRule) {
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "select count(1),card_no_encrypt from b_pos_trans_detail "
           ...
           "and trans_date||trans_time between '%s' and '%s' group by card_no_encrypt", 
                            sMerchId,stRiskRule->sRespCode,sNtTransStamp,sTransStamp);
    tLog(DEBUG, "SQL[%s]", sSqlStr);
