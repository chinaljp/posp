...
int ConvLifeMerch(cJSON *pstJson, int *piFlag) {
    ...
    if (FindLifeMerch(sChanneMerchId, sChannelTermId, sChannelMerchName, sUserCode, sAgentId) < 0) {
        ...
        if( sMerchLevel[0] == '3' ) {
            tLog(DEBUG, "sMerchLevel = [%s]: C类商户[%s]没有开通跳转生活圈,进行按规则自动获取生活圈商户.", sMerchLevel,sMerchId);
            /*取一个本地商户所在市的有效状态的生活圈商户 (此处包含 经济特区 直接跳转本市 )*/
            if ( FindLocalCityLifeMerch(sChanneMerchId, sChannelTermId, sChannelMerchName, sUserCode) < 0 ){
                /*本市没有，随机取本地商户所在省的省会城市的一个有效状态的生活圈商户 （此处 需要排除经济特区 不跳转本省省会城市的 生活圈商户 ）*/
                if( FindCapitalLifeMerch(sChanneMerchId, sChannelTermId, sChannelMerchName, sUserCode) < 0 ) {
