...
int FindMerchInfoById(Merch *pstMerch, char *pcMerchId) {
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "select merch_id,merch_name,merch_pname,status,open_date,expire_date,user_code,agent_id,merch_type,mcc,grade_merch_level "
    ...
    while (OCI_FetchNext(pstRes)) {
        ...
        STRV(pstRes, 11, pstMerch->sMerchLevel);
    }
...
int FindMerchFee(MerchFee *pstMerch, char *pcMerchId) {
    ...
    while (OCI_FetchNext(pstRes)) {
        ...
        DOUV(pstRes, 15, pstMerch->dQDRate);
        DOUV(pstRes, 16, pstMerch->dQCRate);
    }
    if (0 == OCI_GetRowCount(pstRes)) {
