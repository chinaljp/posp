...
int ChkMerchNosign(char * pstMerchId, char * psTermId) {
    char sSqlStr[512];
    int iNum = 0;
    OCI_Resultset *pstRes = NULL;

    snprintf(sSqlStr, sizeof (sSqlStr), "SELECT COUNT(1) FROM B_MERCH A JOIN B_TERM B ON A.USER_CODE = B.USER_CODE \
       WHERE A.MERCH_ID='%s' AND B.TERM_ID='%s' AND A.FREE_FLAG = '1' AND B.FREE_FLAG='1'", pstMerchId, psTermId);
    if (tExecute(&pstRes, sSqlStr) < 0) {
        tReleaseRes(pstRes);
        return -1;
    }
...
int GetFld(int iBitNo, cJSON *pstJson) {
    ...
    UCHAR caMacBuf[1024] = {0}, cOutBuf[1024] = {0}, caMac[8] = {0}, sTransDate[7] = {0};
    ...
    switch (iBitNo) {
        ...
        case 128: /* MAC */
            ...
            if ((iRet = tHsm_Gen_Cups_Mac(caTmp, sTak, cOutBuf, iLen)) < 0) {
...
int SetFld(int iBitNo, cJSON * pstJson) {
    ...
    switch (iBitNo) {
        ...
        case 60: /* 自定义域 */
            ...
            /* && 商户开启免密判断 */
            iRet = ChkMerchNosign(sMerchId, sTermId);
            if (0 == iRet) {
                ...
                if ('2' == sTransCode[5] ||
                    ...
                    if (sInputMode[1] == '7') {
                        ...
                        if (sInputMode[2] == '1') {/* 071说明有密码输入，不开启双免 */
                            tLog(INFO, "免密不开启 F22=071，F60.2.6=0");
                            tStrCpy(sBuf + iLen, "0", 1);
                        } else if ('3' == sTransCode[5] && (DBL_CMP(dTranAmt / 100, dNfcAmount))) {/*  冲正类交易 */
        ...
        case 128: /* MAC */
            ...
            if ((!memcmp(sTransCode, "0G9900", 6)) && (sSecureCtrl[0] == '1')) {
                ...
                if ((iRet = tHsm_Gen_Cups_Mac(caMac, sTak, cOutBuf, iLen)) < 0) {
                    tLog(ERROR, "生成MAC出错[%d].", iRet);
                    return ( -2);
                }
                tDumpHex("计算", caMac, 8);
