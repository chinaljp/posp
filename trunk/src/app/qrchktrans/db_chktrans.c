...
int UPAYEqualProc() {
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "merge into (SELECT * from b_qrcode_settle_detail where chk_flag='N') q \
         using (select * from B_Inline_Tarns_Detail_his where check_flag='N' and trans_code in('02B100','02B200','02W100','02W200','0AB100','0AB200','0AW100','0AW200') ) h \
         on (q.merch_order_no=h.merch_order_no )\
         when matched then \
         update set q.CHK_FLAG='Y'  where q.CHK_FLAG='N'");
    if (tExecute(&pstRes, sSqlStr) < 0) {
        tLog(ERROR, "sql[%s] err!", sSqlStr);
        tRollback();
        return -1;
    }
    tLog(ERROR, "银联支付宝二维码对账流水表[对平]记录数[%d].", tGetAffectedRows(pstRes));
    tCommit();
    tReleaseRes(pstRes);

    snprintf(sSqlStr1, sizeof (sSqlStr1), "merge into (select * from B_Inline_Tarns_Detail_his \
             where check_flag ='N' and trans_code in('02B100','02B200','02W100','02W200','0AB100','0AB200','0AW100','0AW200') and CHECK_FLAG in ('N','M') ) h \
             using b_qrcode_settle_detail q \
            on (q.merch_order_no=h.merch_order_no) \
             when matched then \
            update set  h.CHECK_FLAG='Y',h.channel_fee=q.serve_fee,h.CHANNEL_SETTLE_DATE=q.SETTLE_DATE");
...
int UPAYLongProc() {
    char sSqlStr[1024] = {0};
    OCI_Resultset *pstRes = NULL;
    snprintf(sSqlStr, sizeof (sSqlStr), "UPDATE b_qrcode_settle_detail  set CHK_FLAG='L' \
    where merch_order_no not in (select merch_order_no from B_Inline_Tarns_Detail_his \
    where b_qrcode_settle_detail.merch_order_no=B_Inline_Tarns_Detail_his.merch_order_no  \
    ...
    where b_qrcode_settle_detail.merch_order_no=B_Inline_Tarns_Detail.merch_order_no  \
...
int BsEqualProc() {
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "merge into (SELECT * from b_qrbank_trans_detail where chk_flag='N') q \
         using (select * from B_Inline_Tarns_Detail_his where check_flag='N' and trans_code in('02B100','02B200','02W100','02W200','0AB100','0AB200','0AW100','0AW200') ) h \
         on (q.merch_order_no=h.merch_order_no )\
         when matched then \
         update set q.CHK_FLAG='Y'  where q.CHK_FLAG='N'");
    ...
    tLog(ERROR, "二维码对账流水表[对平]记录数[%d].", tGetAffectedRows(pstRes));
    tCommit();
    tReleaseRes(pstRes);

    snprintf(sSqlStr1, sizeof (sSqlStr1), "merge into (select * from B_Inline_Tarns_Detail_his \
             where check_flag ='N' and trans_code in('02B100','02B200','02W100','02W200','0AB100','0AB200','0AW100','0AW200') and CHECK_FLAG in ('N','M') ) h \
             using b_qrbank_trans_detail q \
            on (q.merch_order_no=h.merch_order_no) \
             when matched then \
            update set  h.CHECK_FLAG='Y',h.channel_fee=q.fee,h.CHANNEL_SETTLE_DATE=q.SETTLE_DATE");
    if (tExecute(&pstRes, sSqlStr1) < 0) {
        tLog(ERROR, "sql[%s] err!", sSqlStr);
        tRollback();
        return -1;
