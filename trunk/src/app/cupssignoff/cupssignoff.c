...
void Cups009100() {
    ...
    SET_STR_KEY(pstJson, "svrid", "009100_P");
    SET_STR_KEY(pstJson, "key", sKey);

    SET_STR_KEY(pstDataJson, "trans_code", "009100");
    ...
    iRet = tRecvMsg("009100_P", sMsg, &iLen, TIMEOUT);
    switch (iRet) {
        case MSG_ERR:
            tLog(ERROR, "获取消息失败.");
            puts("获取消息失败.");
            break;
        case MSG_TIMEOUT:
            tLog(DEBUG, "[%s]等待消息超时[%d].", "009100", TIMEOUT);
            fprintf(stderr, "[%s]等待消息超时[%d].", "009100", TIMEOUT);
            break;
        case MSG_SUCC:
