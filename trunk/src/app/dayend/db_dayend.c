...
int ChkLimitAmt() {
    ...
    if (tIsMonthEnd(sCurDate)) {
        ...
        snprintf(sSqlStr, sizeof (sSqlStr), "update B_MERCH_LIMIT set "
                "D_AMT=0,C_D_AMT=0,D_AMT_M=0,C_D_AMT_M=0,D_WX_AMT=0,D_BB_AMT=0,DQ_AMT=0,CQ_AMT=0");
        if (tExecute(&pstRes, sSqlStr) < 0) {
        ...
        snprintf(sSqlStr, sizeof (sSqlStr), "update B_MERCH_LIMIT set M_BB_AMT=0"
                ...
                ",D_AMT_M=0,M_AMT_M=0,C_D_AMT_M=0,C_M_AMT_M=0,DQ_AMT=0,DQ_AMT_M=0,CQ_AMT=0,CQ_AMT_M=0");
        if (tExecute(&pstRes, sSqlStr) < 0) {
...
int RecoveryLimit(){
   char sSqlStr[512] = {0};
    ...
    snprintf(sSqlStr, sizeof (sSqlStr), "UPDATE B_MERCH_AUTH_LIMIT  SET USABLE_LIMIT = TOTAL_LIMIT,USED_LIMIT= 0, "
            "DEBIT_AVL_AMT = DEBIT_TOTAL_LIMIT, DEBIT_PAID_AMT = 0");
    if (tExecute(&pstRes, sSqlStr) < 0) {
        tLog(ERROR, "sql[%s] err!", sSqlStr);
        return -1;
    }
