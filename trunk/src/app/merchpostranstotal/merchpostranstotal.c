...
int main(int argc, char* pcArgv[]) {
    ...
    while(g_iQutLoop) {
        tGetTime(sCurTime, ":", -1);
        tLog(INFO, "当前时间[%s],助贷通商户累计金额统计时间%s点之后",sCurTime,pcArgv[1]);
        /*取当前时间整点*/
        tStrCpy( sTime, sCurTime, 2 );
        if(atoi(sTime) == atoi(pcArgv[1])) {
            /*助贷通累计交易额 只累计贷记卡交易额*/
            tLog(INFO,"=================累计贷记卡交易额=========================");
            snprintf(sSqlStr,sizeof(sSqlStr),"merge into b_open_merchant o \
                                using (select h.user_code as user_code,sum(h.amount) as amount from b_pos_trans_detail_his h \
                                join b_cups_trans_detail c on c.channel_rrn=h.rrn and c.trans_date=h.trans_date and c.chk_flag='Y' and h.card_type = '1' \
                                join b_open_merchant b on h.user_code = b.user_code and b.create_time < h.create_time \
                                where c.settle_date=to_char(sysdate-1,'YYYYMMDD') group by h.user_code ) p \
...
int BackMoney(){
    ...
    while ( OCI_FetchNext(pstRes) ) { 
     tLog(DEBUG,"OCI_GetRowCount(pstRes)==[%d]",OCI_GetRowCount(pstRes));   
        ...
        if( (memcmp(sCurrentDate,sCycleDate,8)>= 0 && !DBL_CMP(dTransAmt,dFullAmt)) || sRefundStat[0] == 'X' ) {
        ...
        else if( (memcmp(sCurrentDate,sCycleDate,8) <= 0 && DBL_CMP(dTransAmt,dFullAmt)) || (memcmp(sCurrentDate,sCycleDate,8) <= 0 && DBL_EQ(dTransAmt,dFullAmt)) ) {
...
int InlineTotal(char *pcBeforeDate, char *pcSettleDate) {
    ...
    snprintf(sSqlStr,sizeof(sSqlStr),"merge into b_open_merchant o \
                        using (select h.user_code as user_code,sum(h.amount) as amount from b_inline_tarns_detail_his h \
                        join b_open_merchant b on h.user_code = b.user_code and b.create_time < h.create_time \
                                where trans_date between '%s' and '%s' and check_flag = 'Y' and h.trans_code != '0AQ000' group by h.user_code) p \
                        on (o.user_code = p.user_code) \
                        when matched then \
                                update set o.trans_amt =o.trans_amt+p.amount",pcBeforeDate,pcSettleDate);
    tLog(DEBUG, "sql[%s]", sSqlStr);
