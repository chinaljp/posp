...
int FeeAdjust(char *pcType, char *pcTransDate) {
    ...
    if (strncmp(pcType, "pos", 3) == 0) {
        snprintf(sSqlStr, sizeof (sSqlStr), "select id,amount,merch_id,fee,card_type,fee_type,trans_code,user_code,trans_date,trans_time,rrn,merch_order_no from "
                //"(select * from b_pos_trans_detail_his sample(0.5) where fee_flag = 'M'  and rownum=1) ");
                //"(select * from b_pos_trans_detail_his order by dbms_random.value) where fee_flag = 'M' and rownum=1");
                "b_pos_trans_detail_his  where fee_flag = 'M'  and trans_date ='%s' and rownum= decode((select count(*) from b_pos_trans_detail_his where fee_flag = 'M'  and trans_date ='%s' ),0,0,1)",sTransDate, sTransDate ) ;
        ...
        snprintf(sSqlStr, sizeof (sSqlStr), "select id,amount,merch_id,fee,decode(card_type,null,'3',card_type),decode(substr(trans_code,3,1),'W','WX','B','BB','Y',fee_type),trans_code,user_code,trans_date,trans_time,rrn,merch_order_no from "
                "b_inline_tarns_detail_his  where fee_flag = 'M' and rownum=1");
    } else {
        tLog(ERROR, "参数1输入错误.");
        return -2;
    }
...
int AgentProcForeach(int iAgentLevel, int j, cJSON *pstJson) {
    ...
    if (iAgentLevel > 0) {
        ...
        if (sTransCode[2] == 'W' || sTransCode[2] == 'B'||  sTransCode[2] == 'Y') {
            snprintf(sSqlStr, sizeof (sSqlStr), "update b_inline_tarns_detail_his set FEE_FLAG='Y' where id ='%s' ", sId);
        } else {
            snprintf(sSqlStr, sizeof (sSqlStr), "update b_pos_trans_detail_his set FEE_FLAG='Y' where id ='%s' ", sId);
        }
