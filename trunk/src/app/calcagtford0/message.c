...
int FindMerchInfo(char *pcMerchName, char *pcMerchId, char *pcUserCode, char *pcD0Type) {
    char sSqlStr[2048] = {0};
    OCI_Resultset *pstRes = NULL;
    OCI_Statement* pstState = NULL;

    snprintf(sSqlStr, sizeof (sSqlStr), "select  a.merch_name,a.user_code,b.D0_TYPE from B_MERCH a join b_merch_fee b on a.merch_id= b.merch_Id where a.merch_id ='%s' ", pcMerchId);
...
int FindSubCompFeeById(AgentFee *pstAgentFee, char *pcAgentId, char *pcTempFeeType, char *pcD0Type) {
    ...
    if (pcTempFeeType[0] == '0') {
        snprintf(sSqlStr, sizeof (sSqlStr), "select a.agent_id,C_FEE_RATE,C_FEE_MAX,C_FEE_RATIO"
                ...
                ",YC_FEE_RATIO,YD_FEE_RATIO,decode('%s','0',D0_FEE_RATE,'1',S_D0_FEE_RATE) "
                " from B_AGENT_FEE a "
                " join B_AGENT_FEE_TEMP af on af.TEMP_CODE=a.FEE_TEMP_CODE"
                " where a.agent_id='%s'", pcD0Type,pcAgentId);
        ...
        snprintf(sSqlStr, sizeof (sSqlStr), "select a.agent_id,S_C_FEE_RATE,S_C_FEE_MAX,S_C_FEE_RATIO"
                ...
                ",S_YC_FEE_RATIO,S_YD_FEE_RATIO,decode('%s','0',D0_FEE_RATE,'1',S_D0_FEE_RATE)"
                " from B_AGENT_FEE a "
                " join B_AGENT_FEE_TEMP af on af.TEMP_CODE=a.FEE_TEMP_CODE"
                " where a.agent_id='%s'", pcD0Type,pcAgentId);
    }
...
int FindAgentFeeById(AgentFee *pstAgentFee, char *pcAgentId, char *pcD0Type) {
    char sMerchId[] = {0}, sSqlStr[2048] = {0};
    OCI_Resultset *pstRes = NULL;
    OCI_Statement* pstState = NULL;
    /* 查找商户所属代理商 */
...
int D0FeePro(char *pcTransDate) {
    ...
    char sD0Type[1 + 1] = {0}, sMerchId[15 + 1] = {0}, sId[32 + 1] = {0}, sMerchName[500] = {0};
    ...
    if (FindMerchInfo(sMerchName, sMerchId, sUserCode, sD0Type) < 0) {
        tLog(DEBUG, "未找到商户[%s]的信息.", sMerchId);
    }
    ...
    SET_STR_KEY(pstTransJson, "D0_type", sD0Type);
...
int AgentProcForeach(int iAgentLevel, int j, cJSON *pstJson) {
    ...
    char sType[1 + 1] = {0}, sFeeType[1 + 1] = {0}, sAgentlevel[2 + 1] = {0}, sD0Type[1 + 1] = {0}, sGradeAgentLevel[3 + 1] = {0};
    ...
    GET_STR_KEY(pstTransJson, "D0_type", sD0Type);
    //获取代理商信息      
    ...
    if (sAgentlevel[0] > '0') {
        if (FindAgentFeeById(&stAgentFee, sAgentId, sD0Type) < 0) {
            tLog(ERROR, "获取代理商[%s]费率失败.", sAgentId);
        }
    } else {
        GET_STR_KEY(pstAgentJson, "temp_fee_type", sTempFeeType);
        if (FindSubCompFeeById(&stAgentFee, sAgentId, sTempFeeType, sD0Type) < 0) {
            tLog(ERROR, "获取代理商[%s]费率失败.", sAgentId);
        }
    }
    //计算商户所属代理商分润以及成本
