...
int CalcMerchFeeNfc(char *pcMcc,double dAmount) {
    ...
    /* 日常消费mcc */
    /* 交易金额 */
    //if (DBL_CMP(dNfcAmount, dAmount) && 1 == IsSaleMcc(sMcc)) {  //银联二维码 商户 交易时， 如果交易金额小于1000 手续费类型为云闪付 YN
    if (DBL_CMP(dNfcAmount, dAmount)) {
        return 1;
    }
...
int MerchUpFee( char *pcMerchId,char *pcChannelMerchId,char *pcOrderNo,char *pcCardType,char *pcAmount ) {
    ...
    strcpy(sChannelMerchId,pcChannelMerchId);
    ...
    if( sChannelMerchId[0] != '\0' && memcmp(sMerchId,sChannelMerchId,15) ) {
        tLog(WARN,"本次银联二维码交易已跳转生活圈,订单号sOrderNo[%s]",sOrderNo);
        tStrCpy(sMerchType,"0",1);
        //解析生活圈商户MCC
        tStrCpy(sMcc,sChannelMerchId+7,4);
    }
    else {
        tLog(WARN,"本次银联二维码交易不跳转生活圈,订单号sOrderNo[%s]",sOrderNo);
